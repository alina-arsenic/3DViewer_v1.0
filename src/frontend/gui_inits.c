...
void builder_init(widgets_core* w_core, const char* path_xml) {
  ...
  if (!gtk_builder_add_from_file(w_core->ui_builder, path_xml, &w_core->err)) {
    g_critical("Cant find builder UI : %s", w_core->err->message);
    g_error_free(w_core->err);
  }
  // initialising main window of application
  w_core->window_main =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "window_main"));
  w_core->window_help =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "window_help"));
  // add event to main window to handle focus changing(for entry ports)
  gtk_widget_add_events(w_core->window_main, GDK_FOCUS_CHANGE_MASK);
}

void widget_init(widgets_core* w_core, widgets_controls* ctrls) {
  ctrls->box_ctrls =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "box_controls"));
  ...
  ctrls->path_entry =
      GTK_ENTRY(gtk_builder_get_object(w_core->ui_builder, "entry_file_path"));
  ctrls->b_model_select = GTK_WIDGET(
      gtk_builder_get_object(w_core->ui_builder, "button_file_select"));

  ctrls->model_grid_menu =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "grid_model_info"));
  ...
  ctrls->rotat_x.entry =
      GTK_ENTRY(gtk_builder_get_object(w_core->ui_builder, "entry_rotation_x"));
  ...
  ctrls->rotat_y.entry =
      GTK_ENTRY(gtk_builder_get_object(w_core->ui_builder, "entry_rotation_y"));
  ...
  ctrls->rotat_z.entry =
      GTK_ENTRY(gtk_builder_get_object(w_core->ui_builder, "entry_rotation_z"));
  ...
  ctrls->scale_grid_menu =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "grid_menu_scale"));
  ...
  ctrls->scale.entry =
      GTK_ENTRY(gtk_builder_get_object(w_core->ui_builder, "entry_scaling"));
  ctrls->scale.scale =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "scale_scaling"));
  ctrls->scale.adj =
      GTK_ADJUSTMENT(gtk_builder_get_object(w_core->ui_builder, "adj_scaling"));

  ctrls->b_hide_ctrl = GTK_WIDGET(
      gtk_builder_get_object(w_core->ui_builder, "button_hide_controls"));
  ctrls->b_help =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "button_help"));
}
...
void glarea_init(widgets_core* w_core, render_data* render) {
  render->glarea =
      GTK_WIDGET(gtk_builder_get_object(w_core->ui_builder, "gl_drawing_area"));
}

void connect_control_group(controls_group* ctrl_group) {
  g_signal_connect(ctrl_group->adj, "value-changed", G_CALLBACK(on_adj_changed),
                   ctrl_group);
...
void signals_connect(widgets_core* w_core, widgets_controls* ctrls,
  ...
  g_signal_connect(w_core->window_main, "destroy", G_CALLBACK(gtk_main_quit),
                   NULL);
  ...
  g_signal_connect(ctrls->b_help, "pressed", G_CALLBACK(on_btn_pressed_help),
                   w_core);
...
void glarea_signals_connect(render_data* render, widgets_controls* ctrls) {
  ...
  g_signal_connect(render->glarea, "realize", G_CALLBACK(on_glarea_realize),
                   render);
  g_signal_connect(render->glarea, "render", G_CALLBACK(on_glarea_render),
                   render);
  g_signal_connect(render->glarea, "resize", G_CALLBACK(on_glarea_resize),
                   render);

  g_signal_connect(render->glarea, "scroll-event", G_CALLBACK(on_glarea_scroll),
                   ctrls);
...
void set_css_style(widgets_core* w_core, const char* path_css) {
  ...
  gtk_style_context_add_provider_for_screen(
      gdk_screen_get_default(), GTK_STYLE_PROVIDER(provider_file),
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
}

void set_model_info(model model, widgets_controls* ctrls) {
  char vertices_buf[FILENAME_LENGTH], edges_buf[FILENAME_LENGTH];
  gtk_label_set_label(ctrls->model_name, file_from_path((char*)model.path));
