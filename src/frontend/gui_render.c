...
int modelLoad(model *model) {
  ...
  model->scene = 0;
  model->scene =
      aiImportFile(model->path, aiProcessPreset_TargetRealtime_MaxQuality);
  if (model->scene == 0) {
    code = 1;
...
int buffersInit(model *model) {
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(struct aiVector3D) * 2,
                        (GLvoid *)0);
  glEnableVertexAttribArray(0);
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(struct aiVector3D) * 2,
                        (GLvoid *)(sizeof(struct aiVector3D)));
...
void modelDraw(render_data *render) {
  ...
  vec3 center_vec = {render->model->center.x, -render->model->center.y,
                     -render->model->center.z};
  ...
  vec3 trans_vec = {
      render->model->trans.x + buf_x_t * render->model->scale,
      render->model->trans.y + buf_y_t * render->model->scale,
      ((render->model->trans.z + buf_z_t * render->model->scale_z)) /
          (1 + buf_scale / 20)};
  glm_translate(view, trans_vec);

  mat4 projection;
  glm_translate_make(projection, base_vec);
  glm_perspective(45.0f, (GLfloat)render->width / render->height, 0.1f, 4000.0f,
                  projection);
  ...
  GLint projectionLoc =
      glGetUniformLocation(render->shaderProgram, "projection");
  glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, (const GLfloat *)projection);

  glUseProgram(render->shaderProgram);

  glBindVertexArray(render->model->VAO);
  glDrawElements(GL_TRIANGLES, render->model->faces_count * 3, GL_UNSIGNED_INT,
                 0);
